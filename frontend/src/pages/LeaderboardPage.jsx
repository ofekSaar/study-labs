...
const LeaderboardEntry = ({ entry, index }) => {
    ...
    return (
        ...
        >
            {/* Rank */}
            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 text-sm font-black border-b-2 shadow transition-all ${
                rankStyle
            ...
            {/* Avatar */}
            <div className={`w-11 h-11 rounded-2xl flex items-center justify-center text-2xl flex-shrink-0 border transition-all ${
                entry.isYou
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                    <span className={`text-sm font-extrabold truncate ${
                        entry.isYou ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-800 dark:text-white/95'
                    }`}>
                        {entry.name}
                    </span>
                    {entry.isYou && (
                ...
                {entry.level && (
                    <p className="text-[10px] text-slate-400 dark:text-white/35 font-medium mt-0.5">Level {entry.level}</p>
                )}
            </div>
            ...
            <div className="flex items-center gap-1.5 bg-emerald-500/8 dark:bg-emerald-500/12 border border-emerald-500/20 px-3 py-1.5 rounded-xl flex-shrink-0">
                ...
                <span className="text-[11px] font-black text-emerald-600 dark:text-emerald-400">{entry.xp.toLocaleString()} XP</span>
            </div>
        </motion.div>
    );
};
...
const LeaderboardPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchCourse = async () => {
            setIsLoading(true);
            try {
                const { data } = await api.get(`/api/progress/course/${selectedCourseId}/leaderboard?period=${period}`);
                const mapped = (data.leaderboard || []).map((e, idx) => ({
                    ...e,
                    avatar: e.isYou ? currentUserEmoji : MOCK_EMOJIS[idx % MOCK_EMOJIS.length],
                }));
        ...
        <StudentLayout title="Leaderboard">
            ...
            <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
                ...
                <div className="absolute top-0 left-1/3 w-96 h-96 rounded-full pointer-events-none"
                    style={{ background: 'radial-gradient(circle, rgba(245,158,11,0.12) 0%, transparent 70%)', filter: 'blur(50px)' }} />
            ...
            <div className="relative z-[1] p-4 lg:p-8 max-w-4xl mx-auto space-y-6 animate-fade-in">
                ...
                <header className="text-center">
                    <div className="inline-flex items-center justify-center w-16 h-16 rounded-3xl mb-4"
                        style={{ background: 'linear-gradient(135deg, #F59E0B, #D97757)', boxShadow: '0 8px 24px rgba(245,158,11,0.4)' }}>
                        <Trophy size={28} className="text-white" />
                    </div>
                    <h1 className="text-3xl sm:text-4xl font-display font-black text-slate-800 dark:text-white tracking-tight">Leaderboard</h1>
                    <p className="text-slate-500 dark:text-white/50 mt-1 font-medium">See how you rank against your peers</p>
                </header>
                ...
                {myRank && (
                    <motion.div
                        ...
                        style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.10) 0%, rgba(124,58,237,0.08) 100%)' }}
                    >
                        <div className="absolute top-0 right-0 w-32 h-32 rounded-full pointer-events-none opacity-30"
                            style={{ background: 'radial-gradient(circle, rgba(99,102,241,0.4) 0%, transparent 70%)', filter: 'blur(20px)', transform: 'translate(40%, -40%)' }} />
                        <div className="relative flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-widest text-indigo-500 dark:text-indigo-400">Your Rank</p>
                                <p className="text-2xl font-black text-slate-800 dark:text-white">#{myRank.rank}</p>
                            </div>
                            <div className="ml-auto flex items-center gap-1.5 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1.5 rounded-xl">
                                ...
                                <span className="text-sm font-black text-emerald-600 dark:text-emerald-400">{myRank.xp.toLocaleString()} XP</span>
                            </div>
                        </div>
                    </motion.div>
                )}
                    ...
                    <AnimatePresence mode="popLayout">
                        {isLoading ? (
                            Array.from({ length: 8 }).map((_, i) => (
                                <div key={i} className="h-[72px] rounded-2xl bg-slate-100 dark:bg-white/5 animate-pulse" style={{ animationDelay: `${i * 0.05}s` }} />
                            ))
                        ) : entries.length === 0 ? (
                            <motion.div
                                initial={{ opacity: 0 }}
                            ...
                            >
                                ...
                                <p className="text-[11px]">Complete lessons to earn XP and appear here!</p>
                            </motion.div>
                        ) : (
                            entries.map((entry, i) => (
