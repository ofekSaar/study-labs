...
const StudentProfile = () => {
    ...
    const currentAvatar = AVATARS.concat(SHOP_ITEMS.avatars).find((a) => a.id === activeAvatar) || {
        emoji: '🎓',
        name: 'Student',
    };
    const currentTitle = TITLES.concat(SHOP_ITEMS.titles).find((t) => t.id === activeTitle) || {
        name: 'Beginner',
    };
    ...
    return (
        <StudentLayout title="Student Profile">
            ...
            <div className="relative z-[1] p-4 lg:p-6 max-w-[1600px] mx-auto flex flex-col gap-8 animate-fade-in">
                {/* Page Header */}
                <header>
                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-display font-black text-slate-800 dark:text-white drop-shadow-sm tracking-tight">
                        Student Profile
                    </h1>
                    <p className="text-indigo-600 dark:text-indigo-200 mt-1 font-medium">
                        Your gaming identity, achievements, and learning stats.
                    </p>
                </header>

                {/* ── HERO CARD ── */}
                <div
                    className="relative overflow-hidden rounded-3xl border border-white/10 shadow-2xl p-6 lg:p-8"
                    style={{
                        background:
                            'linear-gradient(135deg, #1e1b4b 0%, #312e81 35%, #4c1d95 65%, #1e293b 100%)',
                    }}
                >
                    ...
                    <div className="relative z-10 flex flex-col lg:flex-row gap-6 items-start lg:items-center">
                        ...
                        <div className="flex items-center gap-5 flex-shrink-0">
                            <div className="relative">
                                <div
                                    className={`w-24 h-24 lg:w-28 lg:h-28 rounded-3xl bg-white/10 backdrop-blur border-2 border-white/20 flex items-center justify-center text-5xl lg:text-6xl select-none shadow-2xl avatar-frame-${activeFrame}`}
                                >
                                    {currentAvatar.emoji}
                                </div>
                                <motion.div
                                ...
                                {[
                                    {
                                        Icon: Trophy,
                                        label: `Level ${level}`,
                                        cls: 'text-amber-300  bg-amber-500/20  border-amber-500/30',
                                    },
                                    {
                                        Icon: Zap,
                                        label: `${stats.total_xp || 0} XP`,
                                        cls: 'text-indigo-300 bg-indigo-500/20 border-indigo-500/30',
                                    },
                                    {
                                        Icon: Flame,
                                        label: `${stats.streak || 0}d streak`,
                                        cls: 'text-orange-300 bg-orange-500/20 border-orange-500/30',
                                    },
                                    {
                                        Icon: BookOpen,
                                        label: `${stats.lessons_completed || 0} lessons`,
                                        cls: 'text-emerald-300 bg-emerald-500/20 border-emerald-500/30',
                                    },
                                ].map((chip) => (
                                    <span
                                        key={chip.label}
                                        className={`inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full border ${chip.cls}`}
                                    >
                                        <chip.Icon size={11} />
                                        {chip.label}
                                    </span>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {[
                                    ...
                                    },
                                    {
                                        ...
                                        label: 'XP Boosts',
                                        value: xpBoosts,
                                        cls: 'border-indigo-500/30 bg-indigo-500/10 text-indigo-300',
                                    },
                                    {
                                        emoji: '🥶',
                                        label: 'Freezes',
                                        value: weekendFreezes,
                                    ...
                                    >
                                        ...
                                        <div>
                                            ...
                                            <div className="text-[9px] text-white/40 font-bold uppercase leading-tight">
                                                {label}
                                            </div>
                                        </div>
                                    </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                    {statCards.map((card, i) => (
                        ...
                        >
                            ...
                            <span className="text-[9px] font-bold text-slate-400 dark:text-white/40 uppercase tracking-wider mt-1.5">
                                {card.label}
                            </span>
                        </motion.div>
                    ))}
                ...
                <div className="bg-white/80 dark:bg-white/5 border border-slate-200/80 dark:border-white/10 rounded-3xl p-6 shadow-md relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <div className="flex items-center justify-between mb-5">
                            ...
                            <span className="text-[10px] font-black text-slate-400 dark:text-white/30 uppercase tracking-wider">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            {displayQuests.map((quest) => {
                                ...
                                const pct = Math.min(
                                    100,
                                    Math.round((progress / quest.target) * 100)
                                );
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {isClaimed && (
                                            <p className="mt-3 text-center text-xs font-black text-slate-400 dark:text-white/30">
                                                ✓ Claimed
                                            </p>
                                        )}
                                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white/80 dark:bg-white/5 border border-slate-200/80 dark:border-white/10 rounded-3xl p-6 shadow-md relative overflow-hidden">
                        ...
                        <h3 className="font-display font-bold text-lg text-slate-800 dark:text-white mb-6 relative z-10">
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-7 gap-4 relative z-10">
                            {AVATARS.concat(SHOP_ITEMS.avatars).map((avatar) => {
                                const isUnlocked =
                                    ...
                                    (avatar.levelReq !== undefined && level >= avatar.levelReq);
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {isActive && (
                                            <Check
                                                size={10}
                                                className="absolute top-1.5 right-1.5 stroke-[3] text-white/80"
                                            />
                                        )}
                                    </button>
                                );
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3.5 relative z-10">
                            {TITLES.concat(SHOP_ITEMS.titles).map((title) => {
                                const isUnlocked =
                                    ...
                                    (title.levelReq !== undefined && level >= title.levelReq);
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {isActive && (
                                            <Check
                                                size={10}
                                                className="absolute top-2 right-2 stroke-[3]"
                                            />
                                        )}
                                    </button>
                                );
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 relative z-10">
                            {FRAMES.map((frameItem) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div
                                            className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl bg-slate-100 dark:bg-slate-800 avatar-frame-${frameItem.id}`}
                                        >
                                            {currentAvatar.emoji}
                                        </div>
                                        <span
                                            className={`text-[10px] font-bold text-center leading-tight ${isActive ? 'text-white' : 'text-slate-600 dark:text-white/60'}`}
                                        >
                                            {frameItem.name}
                                        </span>
                                        {!isUnlocked && (
                                        ...
                                        {isActive && (
                                            <Check
                                                size={10}
                                                className="absolute top-2 right-2 stroke-[3] text-white"
                                            />
                                        )}
                                    </button>
                                );
                ...
                <div className="bg-white/80 dark:bg-white/5 border border-slate-200/80 dark:border-white/10 rounded-3xl p-6 shadow-md relative overflow-hidden">
                    ...
                    <div className="relative z-10 mb-6">
                        ...
                        <p className="text-slate-500 dark:text-white/50 text-sm mt-0.5">
                            Unlock exclusive avatars and titles as you level up.
                        </p>
                    </div>
                    ...
                    <div className="relative z-10 overflow-x-auto pb-2">
                        <div className="flex items-center min-w-max">
                            {LEVEL_MILESTONES.map((milestone, idx) => {
                                ...
                                const isNext =
                                    !isUnlocked &&
                                    (idx === 0 || level >= LEVEL_MILESTONES[idx - 1]?.level);
                                const connectorFilled = isUnlocked;

                                return (
                                    <React.Fragment key={milestone.level}>
                                        <div
                                            className={`relative flex flex-col items-center text-center w-36 p-4 rounded-2xl border-2 transition-all ${
                                                isUnlocked
                                        ...
                                        >
                                            ...
                                            <div
                                                className={`w-9 h-9 rounded-full flex items-center justify-center text-[10px] font-black mb-3 shadow-sm ${
                                                    isUnlocked
                                            ...
                                            <span
                                                className={`text-3xl mb-2 select-none transition-all ${isUnlocked ? '' : 'grayscale opacity-50'}`}
                                            >
                                                {milestone.emoji}
                                            </span>
                                        ...
                                        {idx < LEVEL_MILESTONES.length - 1 && (
                                            <div
                                                className={`h-1.5 w-8 mx-1 rounded-full flex-shrink-0 transition-all ${
                                                    connectorFilled
