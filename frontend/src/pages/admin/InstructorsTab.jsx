...
function SummaryCard({ icon, label, value, accent = 'orange' }) {
    ...
    return (
        <div className="bg-white dark:bg-white/[0.03] border border-slate-200/60 dark:border-white/[0.06] rounded-2xl p-4 flex items-center gap-3">
            <div
                className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${ACCENT[accent]}`}
            >
                {icon}
            </div>
            <div>
    ...
    return (
        <div className="space-y-5">
            ...
            <form onSubmit={handleSearch} className="flex gap-2">
                ...
                <button
                    type="submit"
                    className="px-4 py-2.5 rounded-xl bg-orange-500 text-white text-sm font-bold hover:bg-orange-400 transition"
                >
                    Search
                </button>
            </form>
                            ...
                            <thead>
                                <tr className="border-b border-slate-200/60 dark:border-white/[0.06]">
                                    {[
                                        ...
                                        <th
                                            key={h}
                                            className="px-4 py-3 text-left text-[10px] font-black text-slate-400 dark:text-white/25 uppercase tracking-widest"
                                        >
                                            {h}
                                        </th>
                                    ))}
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                <span className="font-bold text-slate-700 dark:text-white/70">
                                                    {inst.stats?.totalCourses ?? 0}
                                                </span>
                                                {inst.stats?.failedCourses > 0 && (
                                                    <span className="text-[10px] text-red-400 font-medium">
                                                        ({inst.stats.failedCourses} failed)
                                                    </span>
                                                )}
                                            </div>
                                        </td>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <div className="flex-1 max-w-[80px] h-1.5 rounded-full bg-slate-100 dark:bg-white/10 overflow-hidden">
                                                        <div
                                                            ...
                                                            style={{
                                                                width: `${inst.stats.successRate}%`,
                                                            }}
                                                        />
                                                    </div>
                                                    <span className="text-xs font-bold text-slate-600 dark:text-white/50">
                                                        {inst.stats.successRate}%
                                                    </span>
                                                </div>
                                            ) : (
