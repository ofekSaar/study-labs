...
const OverviewTab = () => {
    ...
    const roleMap = Object.fromEntries(
        (data?.roleBreakdown ?? []).map((r) => [r._id ?? 'none', r.count])
    );
    const statusMap = Object.fromEntries(
        (data?.coursesByStatus ?? []).map((s) => [s._id ?? 'unknown', s.count])
    );
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white dark:bg-white/[0.03] border border-slate-200/60 dark:border-white/[0.06] rounded-2xl p-5">
                    <h3 className="font-black text-slate-800 dark:text-white text-sm mb-4">
                        Users by Role
                    </h3>
                    {Object.entries(roleMap).map(([role, count]) => (
                        <div key={role} className="mb-3">
                            <div className="flex justify-between text-xs mb-1">
                                <span
                                    className={`font-bold capitalize px-2 py-0.5 rounded-full border text-[10px] ${ROLE_COLORS[role] ?? 'text-white/50'}`}
                                >
                                    {role || 'No role'}
                                </span>
                                <span className="text-slate-500 dark:text-white/40 font-bold">
                                    {count}
                                </span>
                            </div>
                ...
                <div className="bg-white dark:bg-white/[0.03] border border-slate-200/60 dark:border-white/[0.06] rounded-2xl p-5">
                    ...
                    {Object.entries(statusMap).map(([status, count]) => (
                        <div key={status} className="mb-3">
                            <div className="flex justify-between text-xs mb-1">
                                <span className="font-bold text-slate-600 dark:text-white/60 capitalize">
                                    {status}
                                </span>
                                <span className="text-slate-500 dark:text-white/40 font-bold">
                                    {count}
