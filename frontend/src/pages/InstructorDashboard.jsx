...
import {
    Users, BookOpen, PlusCircle, TrendingUp, CheckCircle, Clock,
    BarChart2, GraduationCap, ChevronRight, Check, XCircle,
    AlertCircle, Zap, Bell,
} from 'lucide-react';
...
const STATUS_STYLES = {
    ready:      { cls: 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border-emerald-500/25', label: 'Ready' },
    failed:     { cls: 'bg-red-500/15 text-red-500 dark:text-red-400 border-red-500/25', label: 'Failed' },
    generating: { cls: 'bg-blue-500/15 text-blue-500 dark:text-blue-400 border-blue-500/25', label: 'Generating' },
    pending:    { cls: 'bg-slate-400/15 text-slate-500 dark:text-slate-400 border-slate-400/25', label: 'Pending' },
};
    ...
    return (
        ...
        >
            <div
                ...
                style={{ background: `radial-gradient(circle at 30% 30%, ${glows[color]} 0%, transparent 70%)` }}
            />
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center border ${colors[color]} relative`}>
                {icon}
                {pulse && <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white dark:border-[#16131A] animate-pulse" />}
            </div>
            <div className="relative">
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-white/35 mb-0.5">{label}</p>
...
const ActionCard = ({ label, sub, icon, onClick, gradient }) => (
    ...
    >
        ...
        <div className="min-w-0 flex-1">
            ...
            {sub && <p className="text-[11px] text-slate-400 dark:text-white/35 mt-0.5 leading-tight">{sub}</p>}
        </div>
        <ChevronRight size={14} className="text-slate-300 dark:text-white/20 group-hover:text-slate-500 dark:group-hover:text-white/50 transition-colors flex-shrink-0" />
    </button>
);
...
const RecentCourseRow = ({ course, onClick }) => {
    ...
    const date = course.createdAt ? new Date(course.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) : '—';
    return (
        <button
            onClick={onClick}
        ...
        >
            ...
            <div className="min-w-0 flex-1">
                <p className="text-sm font-bold text-slate-800 dark:text-white truncate leading-tight">{course.title}</p>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <span className={`text-[10px] font-black px-2 py-0.5 rounded-lg border ${s.cls}`}>{s.label}</span>
            </div>
        </button>
    );
};
...
const PendingRow = ({ enrollment, onApprove, onDeny, loadingId }) => {
    ...
    return (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
            ...
            <div className="min-w-0 flex-1">
                <p className="text-[12px] font-bold text-slate-800 dark:text-white truncate leading-tight">{enrollment.student?.name}</p>
                <p className="text-[10px] text-slate-400 dark:text-white/35 truncate">{enrollment.course?.title}</p>
            </div>
...
const SectionCard = ({ title, icon, action, children }) => (
    <div className="bg-white dark:bg-white/5 border border-slate-200/80 dark:border-white/10 rounded-2xl overflow-hidden" style={{ boxShadow: '0 4px 16px rgba(0,0,0,0.04)' }}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-white/8">
            <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-black uppercase tracking-widest text-slate-600 dark:text-white/60">{title}</span>
            </div>
            {action}
        </div>
        <div>{children}</div>
    ...
    return (
        <InstructorLayout title="Instructor Dashboard">
            ...
            <div className="relative z-[1] space-y-6 p-4 sm:p-6 md:p-8 pb-32 max-w-[1200px] mx-auto">
                ...
                {/* ── Welcome Banner ── */}
                <div className="relative overflow-hidden rounded-2xl p-5 sm:p-6 border border-purple-500/20 dark:border-purple-500/25"
                    style={{ background: 'linear-gradient(135deg, rgba(124,58,237,0.08) 0%, rgba(217,119,87,0.06) 100%)' }}>
                    <div className="absolute top-0 right-0 w-64 h-64 rounded-full opacity-30 pointer-events-none"
                        style={{ background: 'radial-gradient(circle, rgba(124,58,237,0.3) 0%, transparent 70%)', filter: 'blur(30px)', transform: 'translate(30%, -30%)' }} />
                    <div className="relative">
                        ...
                        {pendingCount > 0 && (
                            ...
                            >
                                <Bell size={11} />
                                {pendingCount} enrollment request{pendingCount !== 1 ? 's' : ''} waiting
                ...
                {isLoading && !instructorStats ? (
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
                        <KpiCard label="Active Courses"   value={fmt(g.readyCourses)}               icon={<BookOpen size={16} />}    color="purple" />
                        <KpiCard label="Total Students"   value={fmt(e.totalStudents)}               icon={<Users size={16} />}       color="orange" />
                        <KpiCard label="Pending Requests" value={fmt(pendingCount)}                  icon={<Bell size={16} />}        color="amber"  pulse={pendingCount > 0} />
                        <KpiCard label="Avg Completion"   value={fmt(eng.avgCompletionRate, '%')}    icon={<TrendingUp size={16} />}  color="green" />
                        <KpiCard label="Success Rate"     value={fmt(g.successRate, '%')}            icon={<Zap size={16} />}         color="blue" />
                    </div>
                )}
                        ...
                        {(g.totalCourses > 0) && (
                            <SectionCard title="Course Health" icon={<CheckCircle size={13} />}>
                                <div className="p-4 space-y-2.5">
                                    {[
                                        { label: 'Ready',      count: g.readyCourses ?? 0,      total: g.totalCourses, bar: 'bg-emerald-500', text: 'text-emerald-600 dark:text-emerald-400' },
                                        { label: 'Generating', count: g.generatingCourses ?? 0,  total: g.totalCourses, bar: 'bg-blue-500',    text: 'text-blue-600 dark:text-blue-400' },
                                        { label: 'Failed',     count: g.failedCourses ?? 0,      total: g.totalCourses, bar: 'bg-red-500',     text: 'text-red-500 dark:text-red-400' },
                                    ].map(({ label, count, total, bar, text }) => (
                                        <div key={label}>
                                            <div className="flex justify-between text-[11px] mb-1">
                                                <span className="font-bold text-slate-500 dark:text-white/40">{label}</span>
