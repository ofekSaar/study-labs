    ...
    persist(
        (set, get) => {
            ...
            return {
                        ...
                        if (res && res.data) {
                            ...
                            try {
                                const histRes = await api.get(
                                    '/api/progress/gamification/purchase-history'
                                );
                ...
                setTriggerConfetti: (reason) =>
                    set({ triggerConfetti: true, confettiReason: reason }),
                ...
                checkLevelUp: (previousXP, newXP) => {
                    ...
                    const newlyUnlockedAvatars = AVATARS.filter(
                        (a) => a.levelReq <= newLevel && !get().unlockedAvatars.includes(a.id)
                    ).map((a) => a.id);
                    const newlyUnlockedTitles = TITLES.filter(
                        (t) => t.levelReq <= newLevel && !get().unlockedTitles.includes(t.id)
                    ).map((t) => t.id);
                ...
                logActivity: () => {
                    ...
                    if (!activityLog.includes(today)) {
                        const pruned = [...activityLog, today].filter((d) => {
                            const diff =
                ...
                generateDailyChallenge: () => {
                    ...
                    const challenges = [
                        ...
                        {
                            id: 'no_mistakes',
                            title: 'Answer 5 questions without mistakes',
                            xp: 250,
                            icon: '⚡',
                        },
                    ];
                ...
                setLeaderboard: (data) =>
                    set({ leaderboard: data, leaderboardFetchedAt: Date.now() }),
                ...
                initializeQuests: () => {
                    ...
                    if (activeQuests.daily.length > 0 && activeQuests.daily[0]?.date === today)
                        return;
                ...
                buyItem: async (itemType, itemId, cost) => {
                    ...
                    if (itemType === 'avatars' && unlockedAvatars.includes(itemId)) {
                        useToastStore
                            .getState()
                            .success('Already Owned', 'You already own this avatar!', 3000);
                        return false;
                    }
                    if (itemType === 'titles' && unlockedTitles.includes(itemId)) {
                        useToastStore
                            .getState()
                            .success('Already Owned', 'You already own this title!', 3000);
                        return false;
                    }
                    if (itemType === 'themes' && unlockedThemes.includes(itemId)) {
                        useToastStore
                            .getState()
                            .success('Already Owned', 'You already own this theme!', 3000);
                        return false;
                    }
                    if (itemType === 'frames' && unlockedFrames.includes(itemId)) {
                        useToastStore
                            .getState()
                            .success('Already Owned', 'You already own this frame!', 3000);
                        return false;
                    }

                    try {
                        const res = await api.post('/api/progress/gamification/shop/buy', {
                            category: itemType,
                            itemId,
                        });
                        if (res && res.data) {
                            ...
                            if (itemType === 'avatars') {
                                ...
                                useToastStore
                                    ...
                                    .success(
                                        'Purchase Successful!',
                                        `Unlocked Avatar: ${item.emoji} ${item.name}`,
                                        4000
                                    );
