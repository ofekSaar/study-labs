...
const StepGamification = () => {
    ...
    return (
        <div className="space-y-5 animate-fade-in">
            ...
            <div className="flex items-start justify-between">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">
                        Boost student motivation with XP and competition.
                    </p>
                </div>
            ...
            {/* Engagement Score */}
            <div
                className={`rounded-2xl border-2 p-4 transition-all duration-300 ${engagement.bg} ${engagement.border} ${engagement.glow}`}
            >
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className={`text-xl font-display font-black ${engagement.color}`}>
                                {engagement.label}
                            </span>
                        </div>
                    </div>
            ...
            <div className="glass-card rounded-2xl p-5">
                <div className="flex items-center gap-3 mb-5">
                    ...
                    <div>
                        <label
                            htmlFor="wizard-xp-multiplier"
                            className="font-bold text-gray-900 text-sm block"
                        >
                            XP Multiplier
                        </label>
                        <p className="text-xs text-gray-400">
                            Boost rewards for completing this course
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center justify-center mb-5">
                    <div
                        className={`relative flex flex-col items-center justify-center w-32 h-32 rounded-full transition-all duration-300
                        ${
                            xpMultiplier >= 1.5
                    ...
                    >
                        <span
                            className={`text-3xl font-display font-black ${xpMultiplier >= 1.5 ? 'text-white' : 'text-gray-600'}`}
                        >
                            {xpMultiplier.toFixed(1)}x
                        </span>
                        {xpMultiplier > 1.0 && (
                            <span
                                className={`text-[10px] font-bold mt-0.5 ${xpMultiplier >= 1.5 ? 'text-orange-100' : 'text-gray-500'}`}
                            >
                                BONUS XP
                            </span>
                        )}
                ...
                <div className="flex items-center justify-center gap-4 mb-4 text-sm">
                    ...
                    <div className="text-center">
                        ...
                        <p
                            className={`font-black text-lg ${xpMultiplier > 1 ? 'text-studylabs-orange' : 'text-gray-500'}`}
                        >
                            {boostedXP} XP
                        </p>
                    </div>
            ...
            {/* Leaderboard Toggle */}
            <div
                className={`rounded-2xl border-2 p-4 transition-all duration-300 ${leaderboardEnabled ? 'border-amber-200 bg-amber-50/50' : 'border-gray-100 bg-white'}`}
            >
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        <div
                            className={`p-2.5 rounded-xl transition-colors ${leaderboardEnabled ? 'bg-amber-100 text-amber-600' : 'bg-gray-100 text-gray-400'}`}
                        >
                            <Trophy size={20} />
                        </div>
                        <div>
                            ...
                            <span className="text-xs text-gray-400">
                                Students compete for top spots
                            </span>
                        </div>
                    </div>
                ...
                <div
                    className={`relative rounded-xl overflow-hidden transition-all duration-300 ${!leaderboardEnabled ? 'opacity-40' : ''}`}
                >
                    {!leaderboardEnabled && (
                        <div className="absolute inset-0 backdrop-blur-[2px] z-10 flex items-center justify-center">
                            <span className="text-xs font-bold text-gray-500 bg-white/80 px-3 py-1 rounded-full">
                                Enable to show leaderboard
                            </span>
                        </div>
                    )}
            ...
            <div className="glass-card rounded-2xl p-4">
                ...
                <div className="flex items-center gap-4">
                    <div className="relative">
                        ...
                        {xpMultiplier >= 1.5 && (
                            <div className="absolute -top-1 -right-1 w-5 h-5 bg-amber-400 rounded-full flex items-center justify-center text-[9px]">
                                ⭐
                            </div>
                        )}
                    </div>
                    <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                            Awarded to students who finish all lessons
                        </p>
                        {xpMultiplier >= 1.5 && (
                            <p className="text-xs text-amber-600 font-bold mt-1">
                                ✨ Boosted — earns {boostedXP} XP
                            </p>
                        )}
                    </div>
                </div>
