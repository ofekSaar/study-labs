...
const GameNavItem = ({ icon, label, active, onClick, badge }) => (
    <button
        ...
        style={active ? { boxShadow: '-3px 0 12px rgba(217,119,87,0.2), inset 0 0 20px rgba(217,119,87,0.04)' } : {}}
    >
        ...
        {active && <ChevronRight size={12} className="ml-auto text-orange-500/70 relative shrink-0" />}
    </button>
);

/* ── Sidebar Content ── */
const SidebarContent = ({ location, navigate, courses, selectedCourseId, setSelectedCourse, user, setIsSidebarOpen, setIsSettingsOpen, handleLogout }) => {
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto px-3 py-2 space-y-0.5">
                ...
                <GameNavItem
                    ...
                    onClick={() => { navigate('/my-courses'); setIsSidebarOpen(false); }}
                />
                <GameNavItem
                    icon={<Map size={18} />}
                    label="Find Courses"
                    active={location.pathname === '/enrollments'}
                    onClick={() => { navigate('/enrollments'); setIsSidebarOpen(false); }}
                />
                <GameNavItem
                    icon={<User size={18} />}
                    label="Profile"
                ...
                <GameNavItem
                    ...
                    onClick={() => { navigate('/shop'); setIsSidebarOpen(false); }}
                    badge={coins > 0}
                />
                                    ...
                                    <div className="flex items-center gap-2.5 mb-2.5">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <div className="flex items-center gap-1.5 mt-0.5">
                                                <span className="text-[9px] font-bold text-orange-600 dark:text-orange-400">{completedNodes}/{totalNodes} stages</span>
                                ...
                                {isCourseDropdownOpen && (
                                    <div className="mt-2 p-1.5 bg-slate-100 dark:bg-slate-900 border border-slate-200/80 dark:border-white/5 rounded-2xl space-y-1 shadow-md max-h-48 overflow-y-auto custom-scrollbar">
                                        {courses.map(course => {
                                            ...
                                            const cc = course.nodes?.filter(n => n.status === 'completed').length || 0;
                                                ...
                                                >
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        <p className="text-[9px] font-bold text-slate-400 dark:text-white/30 mt-0.5">
                                                            {cc}/{tc} stages · {course.progress || 0}%
                                                        </p>
                                                    </div>
                                                </button>
                                            );
                        ...
                        <div className="bg-white dark:bg-[#1A1410] rounded-[17px] p-3 space-y-2.5">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="relative flex-shrink-0">
                                    <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-700 flex items-center justify-center text-3xl shadow-md transition-transform duration-300 group-hover/card:scale-105 select-none avatar-frame-${activeFrame}`}>
                                        {currentAvatar.emoji}
                                    </div>
                                    {/* Level badge */}
                                    <div
                                        className="absolute -bottom-1.5 -right-1.5 px-1.5 py-0.5 rounded-lg text-[8px] font-black text-white leading-none select-none"
                                        style={{ background: 'linear-gradient(135deg, #F59E0B, #D97757)', boxShadow: '0 2px 6px rgba(245,158,11,0.5)' }}
                                    >
                                        LV {stats.level || 1}
                                    </div>
                                    {/* Online dot */}
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    <p
                                        ...
                                        style={{ background: 'linear-gradient(135deg, #D97757, #7C3AED)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}
                                    >
                                        {currentTitle.name}
                                    </p>
                                    {/* Mini stat pills */}
                                    <div className="flex items-center gap-1.5 mt-1.5">
                                        <div className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-amber-500/10 dark:bg-amber-500/15 border border-amber-500/20">
                                            ...
                                            <span className="text-[8px] font-black text-amber-600 dark:text-amber-400">{(stats.total_xp || 0).toLocaleString()}</span>
                                        </div>
                                        <div className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-orange-500/10 dark:bg-orange-500/15 border border-orange-500/20">
                                            ...
                                            <span className="text-[8px] font-black text-orange-600 dark:text-orange-400">{stats.streak || 0}d</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="flex items-center gap-1">
                    ...
                    {user?.roles?.includes('instructor') && (
                        ...
                        >
                            ...
                            <span className="text-[8px] font-black uppercase tracking-wide">Teacher</span>
                        </button>
                    )}
                    <button
