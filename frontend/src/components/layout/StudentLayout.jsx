...
const NavItem = ({ icon, label, active, onClick }) => (
    <button
        ...
        style={active ? { background: 'rgba(79,110,247,0.25)', border: '1px solid rgba(79,110,247,0.4)' } : {}}
    >
        {active && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-r-full bg-indigo-400" />}
        <span className={`transition-colors ${active ? 'text-indigo-300' : 'text-white/40 group-hover:text-white/70'}`}>
            {icon}
        </span>
        <span>{label}</span>
    ...
    const SidebarContent = () => (
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1">
                <div className="px-4 mb-2 text-[10px] font-bold text-white/25 uppercase tracking-widest">Discover</div>
                ...
                {courses.length > 0 && (
                    <>
                        <div className="px-4 mt-6 mb-2 text-[10px] font-bold text-white/25 uppercase tracking-widest">Active Roadmaps</div>
            ...
            <div className="p-4 border-t border-white/8">
                ...
                <div className="flex items-center gap-3 px-3 py-3 mb-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors cursor-default">
                    ...
                    <div className="min-w-0 flex-1">
                        <p className="text-sm font-bold text-white truncate leading-tight">{user?.name || 'Student'}</p>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            ...
                            <div className="flex items-center gap-1 text-indigo-400" title="Total XP">
                ...
                <div className="space-y-0.5">
                    {user?.roles?.includes('instructor') && (
                        <button
                            ...
                            className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium text-purple-300 hover:bg-purple-500/15 hover:text-purple-200 transition-all"
                        >
