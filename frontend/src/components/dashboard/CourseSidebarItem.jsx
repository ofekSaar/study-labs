...
const CourseSidebarItem = ({ course, isSelected, onClick }) => {
    return (
        <motion.div
            ...
            className={`cursor-pointer group relative p-3 rounded-xl transition-all duration-300 ${
                isSelected
        ...
        >
            <div className="flex items-center gap-3">
                ...
                <div
                    className={`w-10 h-10 rounded-lg flex items-center justify-center shadow-sm transition-transform group-hover:scale-105 ${
                        isSelected
                            ? `${course.color || 'bg-indigo-600'} text-white`
                ...
                <div className="flex-1 min-w-0">
                    <h4
                        className={`text-sm font-bold truncate ${isSelected ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-white/70 group-hover:text-slate-800 group-hover:dark:text-white/90'}`}
                    >
                        {course.title}
                    </h4>
                        ...
                        {course.nodes && course.nodes.length > 0 && (
                            <>
                                ...
                                <span className="text-indigo-600 dark:text-indigo-300">
                                    {course.nodes.filter((n) => n.status === 'completed').length}/
                                    {course.nodes.length} Stages
                                </span>
                            </>
                        )}
                    </p>
            ...
            {isSelected && (
                ...
                >
                    {(() => {
                        const activeNode =
                            course.nodes.find((n) => n.status === 'active') ||
                            course.nodes.find((n) => n.status !== 'completed');
                        if (activeNode) {
                            return (
