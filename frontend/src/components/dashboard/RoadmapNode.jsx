...
const RoadmapNode = ({ node, onClick, alignment, isSelected }) => {
    ...
    const getNodeStyles = () => {
        if (isCompleted)
            return 'bg-gradient-to-br from-emerald-400 to-teal-500 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.3)] text-white';
        if (isCurrent)
            return 'bg-gradient-to-br from-indigo-500 to-purple-600 border-indigo-600 shadow-[0_0_20px_rgba(99,102,241,0.5)] scale-110 z-10 text-white';
    ...
    return (
        <div className="relative group flex flex-col items-center">
            ...
            >
                <h3
                    className={`font-black text-xs tracking-wider leading-relaxed transition-colors ${
                        isLocked
                ...
                <p
                    className={`text-[9px] font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-widest mt-1.5 flex items-center gap-1 ${
                ...
                {isCompleted &&
                    (() => {
                        ...
                        if (qs && qs.totalAnswerable > 0) {
                            ...
                            return (
                                <div
                                    className={`absolute -top-3.5 bg-white dark:bg-slate-800 px-2 py-0.5 rounded-full shadow-md border scale-90 font-black text-[9px] ${color}`}
                                >
