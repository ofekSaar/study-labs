...
import os
import hashlib
from engine.db import get_db_handle, save_course_to_db, update_course_progress, save_to_staging, check_file_hash, save_file_hash, update_course_in_db
...
async def generate_course(request: GenerateCourseRequest, req: Request):
    try:
        ...
        if not request.isUpdate:
            ...
            if check_file_hash(syllabus_hash):
            ...
            save_file_hash(os.path.basename(request.syllabusPath), syllabus_hash)
        ...
        update_course_progress(request.courseId, f"Parsing {len(target_materials)} course materials...")
        materials_text = []
        for mat_path in target_materials:
            ...
            if check_file_hash(mat_hash):
                logger.error(f"Duplicate material file detected based on SHA256 hash: {os.path.basename(mat_path)}")
                raise HTTPException(status_code=400, detail=f"Duplicate material file detected: {os.path.basename(mat_path)}. Processing aborted.")
            save_file_hash(os.path.basename(mat_path), mat_hash)
        ...
        if request.isUpdate:
            ...
            course_doc = update_course_in_db(request.courseId, course, updated_topics)
        else:
